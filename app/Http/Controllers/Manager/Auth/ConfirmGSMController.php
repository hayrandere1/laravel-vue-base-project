...
class ConfirmGSMController extends Controller
{
    ...
    {
        return __('manager.confirm_gsm.smsContent', ['code' => $code]);
    }

    public function index()
    {
        $sessions['status'] = Session::get('status');
        $sessions['phones'] = Auth()->user()->managerPhones()->pluck('phone', 'id')->toArray();
        return Inertia::render('Manager/Auth/ConfirmGsm',compact('sessions'));
    }

    public function checkGSM()
    {
        return Inertia::render('Manager/Auth/CheckGsm');
    }
    ...
    public function validGSM(Request $request)
    {
        if (Auth::user()->login_code == $request->code && is_numeric($request->code)) {
            if (now() < Auth::user()->login_code_expired) {
                Session::put('manager_gsm_confirm', true);
                Auth::user()->update([
                    'login_code' => 'access',
        ...
        return redirect()->route('manager.login')->with('code_rejected', __('user.auth.gsmValidError.' . $message));

    }

    public function confirm(Request $request)
    {
        $gsm = Auth::user()->managerPhones()->where('id', $request->number)->first();
        if (!empty($gsm)) {
            $gsm = $gsm->phone;
            ...
            return redirect()->route('manager.checkGsm');
