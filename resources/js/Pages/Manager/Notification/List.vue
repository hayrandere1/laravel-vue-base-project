                        ...
                        <template v-slot:item.process="{ item }">
                            ...
                            <template v-else>
                                ...
                                <v-btn
                                    ...
                                    :href="route('manager.notification.show',item.raw.id)"
                                >
                                </v-btn>
                            </template>
                        </template>
...
export default {
    ...
    methods: {
        loadItems({page, itemsPerPage, sortBy}) {
            ...
            axios.get(route('manager.notification.getData'), {
                params: filterDetail
            }).then(response => {
                this.loading = false;
        ...
        deleteItem() {
            ...
            axios.delete(route('manager.notification.destroy', this.deleteData.id)).then(response => {
        ...
        download() {
            // this.$inertia.post(route('manager.notification.download'), this.resource.filter, {preserveState: true})
            // this.loading = true
            let filterDetail = {
                'search': this.filter.search,
            }
            axios.post(route('manager.notification.download'), {
                params: filterDetail
            }).then(response => {
                // this.loading = false;
        ...
        rowDownload(item) {
            console.log(5, item)
            axios.get(route('manager.notification.download', item.id), {responseType: 'blob'})
                .then(response => {
