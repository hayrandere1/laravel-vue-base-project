                        ...
                        <template v-slot:item.process="{ item }">
                            ...
                            <template v-else>
                                ...
                                <v-btn
                                    ...
                                    :href="route('user.notification.show',item.raw.id)"
                                >
                                </v-btn>
                            </template>
                        </template>
...
export default {
    ...
    methods: {
        loadItems({page, itemsPerPage, sortBy}) {
            ...
            axios.get(route('user.notification.getData'), {
                params: filterDetail
            }).then(response => {
                this.loading = false;
        ...
        deleteItem() {
            ...
            axios.delete(route('user.notification.destroy', this.deleteData.id)).then(response => {
        ...
        download() {
            // this.$inertia.post(route('user.notification.download'), this.resource.filter, {preserveState: true})
            // this.loading = true
            let filterDetail = {
                'search': this.filter.search,
            }
            axios.post(route('user.notification.download'), {
                params: filterDetail
            }).then(response => {
                // this.loading = false;
        ...
        rowDownload(item) {
            console.log(5, item)
            axios.get(route('user.notification.download', item.id), {responseType: 'blob'})
                .then(response => {
