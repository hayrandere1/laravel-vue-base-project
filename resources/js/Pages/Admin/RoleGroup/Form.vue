            ...
            <v-card>
                ...
                <v-card-text>
                    <v-form v-on:submit.prevent="save" :disabled="isShow()">
                        ...
                        <v-row>
                            <template v-for="(role,key) in this.form.adminRoles">
...
export default {
    ...
    methods: {
        ...
        allSwitchChange(key) {
            this.adminRoles[key].children.forEach((value, index) => {
                value.checked = this.allSwitches[key];
            });
        },
        save() {
            if (this.isUpdate()) {
                this.form.put(route('admin.admin_role_group.update', this.roleGroup.id));
